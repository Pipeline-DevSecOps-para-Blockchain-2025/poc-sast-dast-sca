/**
 * From Safe Smart Account: https://safe.global/
 * Source: https://github.com/safe-fndn/safe-smart-account/blob/v1.5.0/contracts/Safe.sol
 */

// SPDX-License-Identifier: LGPL-3.0-only
pragma solidity >=0.7.0 <0.9.0;

import { FallbackManager } from "safe-smart-account/base/FallbackManager.sol";
import { GuardManager, ITransactionGuard } from "safe-smart-account/base/GuardManager.sol";
...
import { NativeCurrencyPaymentFallback } from "safe-smart-account/common/NativeCurrencyPaymentFallback.sol";
import { SecuredTokenTransfer } from "safe-smart-account/common/SecuredTokenTransfer.sol";
import { SignatureDecoder } from "safe-smart-account/common/SignatureDecoder.sol";
import { Singleton } from "safe-smart-account/common/Singleton.sol";
import { StorageAccessible } from "safe-smart-account/common/StorageAccessible.sol";
import { SafeMath } from "safe-smart-account/external/SafeMath.sol";
import { ISafe } from "safe-smart-account/interfaces/ISafe.sol";
import {
    ISignatureValidator,
    ISignatureValidatorConstants
} from "safe-smart-account/interfaces/ISignatureValidator.sol";
 ...
 *      - Owners: List of addresses that control the Safe. They are the only ones that can add/remove owners, change the
 * threshold and
 *        approve transactions. Managed in `OwnerManager`.
 *      - Transaction Hash: Hash of a transaction is calculated using the EIP-712 typed structured data hashing scheme.
 *      - Nonce: Each transaction should have a different nonce to prevent replay attacks.
 *      - Signature: A valid signature of an owner of the Safe for a transaction hash.
 *      - Guards: Guards are contracts that can execute pre- and post- transaction checks. There are two types of
 * guards:
 ...
 *      - Modules: Modules are contracts that can be used to extend the write functionality of a Safe. Managed in
 * `ModuleManager`.
 *      - Fallback: Fallback handler is a contract that can provide additional functionality for Safe. Managed in
 * `FallbackManager`. Please read the security risks in the `IFallbackManager` interface.
 *      Note: This version of the implementation contract doesn't emit events for the sake of gas efficiency and
 * therefore requires a tracing node for indexing/
    ...
    ISafe
{
    ...
    function setup(
        ...
        // Emit the setup event optimistically. This ensures that changes such as `addOwner` and `changeThreshold` that
        // are part of the  `to.delegatecall(data)` that happen in the `setupModules` call emit events in order relative
        // to the setup
        // event, making it easier for off-chain indexers to reliably reconstruct the Safe configuration.
        emit SafeSetup(msg.sender, _owners, _threshold, to, fallbackHandler);

        // setupOwners checks if the Threshold is already set, therefore preventing this method from being called more
        // than once
        setupOwners(_owners, _threshold);
        if (fallbackHandler != address(0)) {
            internalSetFallbackHandler(fallbackHandler);
        }
        // As setupOwners can only be called if the contract has not been initialized we don't need a check for
        // setupModules
        setupModules(to, data);

        if (payment > 0) {
            // To avoid running into issues with EIP-170 we reuse the handlePayment function (to avoid adjusting code
            // that has been verified we do not adjust the method itself) baseGas = 0, gasPrice = 1 and gas = payment =>
            // amount = (payment + 0) * 1 = payment
    ...
    function execTransaction(
        ...
        // We require some gas to emit the events (at least 2500) after the execution and some to perform code until the
        // execution (500) We also include the 1/64 in the check that is not sent along with a call to counteract
        ...
        // Use scope here to limit variable lifetime and prevent `stack too deep` errors
        {
            ...
            // If the gasPrice is 0 we assume that nearly all available gas can be used (it is always more than
            // safeTxGas) We only subtract 2500 (compared to the 3000 before) to ensure that the amount passed is still
            // higher than safeTxGas
            success = execute(to, value, data, operation, gasPrice == 0 ? (gasleft() - 2500) : safeTxGas);
            gasUsed = gasUsed.sub(gasleft());
            // If no safeTxGas and no gasPrice was set (e.g. both are 0), then the internal tx is required to be
            // successful This makes it possible to use `estimateGas` without issues, as it searches for the minimum gas
            // where the tx doesn't revert
            ...
            // We transfer the calculated tx costs to the tx.origin to avoid sending it to intermediate contracts that
            // have made calls
            uint256 payment = 0;
            if (gasPrice > 0) {
     ...
     */
    function checkContractSignature(address owner, bytes32 dataHash, bytes memory signatures, uint256 offset)
        internal
        view
    {
        // Check that signature data pointer (s) is in bounds (points to the length of data -> 32 bytes)
        if (offset.add(32) > signatures.length) {
            revertWithError("GS022");
        }
        ...
        /// @solidity memory-safe-assembly
        assembly {
            // The signature data for contract signatures is appended to the concatenated signatures and the offset is
            // stored in s
        ...
        override
    {
        ...
        // This essentially means that for every signature, there's another valid signature (known as ECDSA
        // malleability) Since we have other mechanisms to prevent duplicated signatures (ordered owners array) and
        ...
        for (i = 0; i < requiredSignatures; ++i) {
            ...
            if (v == 0) {
                ...
                // are sent. Here we only check that the pointer is not pointing inside the part that is being processed
                if (uint256(s) < requiredSignatures.mul(65)) {
                    revertWithError("GS021");
                }

                // The contract signature check is extracted to a separate function for better compatibility with formal
                // verification A quote from the Certora team:
                // "The assembly code broke the pointer analysis, which switched the prover in failsafe mode, where it
                // is (a) much slower and (b) computes different hashes than in the normal mode." More info here:
                // https://github.com/safe-global/safe-smart-account/pull/661
                ...
                if (executor != currentOwner && approvedHashes[currentOwner][dataHash] == 0) {
     ...
     *      ⚠️⚠️⚠️ If the caller is an owner of the Safe, it can trivially sign any hash with a pre-approve
     * signature and may reduce the threshold of the signature by 1. ⚠️⚠️⚠️
     * @param dataHash Hash of the data (could be either a message hash or transaction hash).
    ...
    function getTransactionHash(
        ...
        //
        // WARNING: We do not clean potential dirty bits in types that are less than 256 bits (addresses and
        // Enum.Operation) The solidity assembly types that are smaller than 256 bit can have dirty high bits according
        // to the spec (see the Warning in
        // https://docs.soliditylang.org/en/latest/assembly.html#access-to-external-variables-functions-and-libraries).
        // However, we read most of the data from calldata, where the variables are not packed, and the only variable we
        // read from storage is uint256 nonce. This is not a problem, however, we must consider this for potential
        // future changes.
        /* solhint-disable no-inline-assembly */
        /// @solidity memory-safe-assembly
        assembly {
